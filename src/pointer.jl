...
if Base.VERSION >= v"0.6.1"
    ...
    @generated function Base.unsafe_load(p::DevicePtr{T,A}, i::I=1,
        ...
        llvm_f, _ = create_llvmf(eltyp, param_types)
        ...
        call_llvmf(llvm_f, T, Tuple{Ptr{T}, Int}, :((pointer(p), Int(i-one(I)))))
    end
    ...
    @generated function Base.unsafe_store!(p::DevicePtr{T,A}, x, i::I=1,
        ...
        llvm_f, _ = create_llvmf(LLVM.VoidType(jlctx[]), param_types)
        ...
        call_llvmf(llvm_f, Void, Tuple{Ptr{T}, T, Int}, :((pointer(p), convert(T,x), Int(i-one(I)))))
    end
else
