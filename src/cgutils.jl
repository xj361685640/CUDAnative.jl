...
@generated function _asm(::Val{key}, ::Val{side_effects},
    ...
    llvm_f, llvm_ft = create_llvmf(llvm_rettyp, llvm_argtyp)
    ...
    call_llvmf(llvm_f, rettyp, argtyp,
               Expr(:tuple, (:(args[$i]) for i in 1:length(args))...))
end

macro asm(asm::String, constraints::String, side_effects::Bool,
          rettyp::Symbol=:(Void), argtyp::Expr=:(Tuple{}), args...)
    key = hash((asm, constraints))
    asmcache[key] = (asm, constraints)
    return esc(:(CUDAnative._asm(Val{$key}(), Val{$side_effects}(),
                                 Val{$rettyp}(), Val{$argtyp}(),
                                 $(args...))))
end

# shorthand: no side_effects
macro asm(asm::String, constraints::String,
          rettyp::Symbol=:(Void), argtyp::Expr=:(Tuple{}), args...)
    esc(:(CUDAnative.@asm $asm $constraints false $rettyp $argtyp $(args...)))
end

# shorthand: no side_effects or constraints
macro asm(asm::String,
          rettyp::Symbol=:(Void), argtyp::Expr=:(Tuple{}), args...)
    esc(:(CUDAnative.@asm $asm "" $rettyp $argtyp $(args...)))
end
